...
func (pc *ProxyChecker) run(proxies []map[string]any) ([]Result, error) {
	...
	if config.GlobalConfig.TotalSpeedLimit != 0 {
		...
		Bucket = ratelimit.NewBucketWithRate(1e12, 1e11) // 1TB/s, 100GB burst
	}
	...
	// 阶段1：高并发测活
	const aliveConcurrent = 200
	var aliveProxies []map[string]any
	var mu sync.Mutex
	var wg sync.WaitGroup
	tasks := make(chan map[string]any, aliveConcurrent)

	for i := 0; i < aliveConcurrent; i++ {
		wg.Add(1)
		go func() {
			defer wg.Done()
	...
	slog.Info(fmt.Sprintf("测活通过节点数量: %d", len(aliveProxies)))

	// 阶段2：低并发测速/流媒体检测
	const testConcurrent = 20
	var results []Result
	var mu2 sync.Mutex
	var wg2 sync.WaitGroup
	tasks2 := make(chan map[string]any, testConcurrent)

	for i := 0; i < testConcurrent; i++ {
		wg2.Add(1)
		go func() {
			defer wg2.Done()
			for proxy := range tasks2 {
...
// isAlive 测活方法
func (pc *ProxyChecker) isAlive(proxy map[string]any) bool {
